...
int say(int socket, char *s)
{
    ...
    raise(SIGSEGV);   //deliberately raise a segfault
    return res;
}
...
pthread_t make_thread(void *(*entry)(void *), int* shared)
{
    int ret;
    pthread_t thread;
...
void child_code(int shared)
{
  int connect_d = shared;
  printf("%d\n", connect_d);
...
int main(int argc, char *argv[])
{
    ...
    while (1) {
        ...
        child = make_thread(entry, (void*)&connect_d);
